...
function teamKey(value) {
  ...
  return NAME_TO_ABBR[raw.toUpperCase()] || NAME_TO_ABBR[raw] || raw.toUpperCase();
}

function ratingFor(sagarin, teamName) {
  const key = teamKey(teamName);
  if (sagarin.ratings[key] != null) return sagarin.ratings[key];
  if (sagarin.ratings[teamName] != null) return sagarin.ratings[teamName];
  return null;
}
...
export function sagarinForGame(game, sagarin) {
  ...
  const homeRating = ratingFor(sagarin, game.home_team);
  const awayRating = ratingFor(sagarin, game.away_team);
